"""
AI Daily Omniverse — Episode 2 Production (2026-03-25)
...
EPISODE_DIR = Path(__file__).parent.parent / "data" / "episodes" / "2026-03-25"
...
def upload_audio_heygen(audio_path):
    ...
    if resp.status_code == 200:
        data = resp.json()
        # HeyGen returns asset_id in data dict or top-level
        asset_id = data.get("data", {}).get("asset_id") or data.get("asset_id")
        if not asset_id:
            # Try url field
            asset_id = data.get("data", {}).get("url") or data.get("url")
...
def main():
    print("=" * 60)
    print("  AI Daily Omniverse -- Episode 2 Production")
    ...
    print()
    print("Next: python scripts/assemble_newscast.py 2026-03-25")
    print("=" * 60)
