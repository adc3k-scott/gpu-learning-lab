---
name: Scott Tomsu
description: Founder, Advantage Design & Construction — full bio and background
type: user
---

# Scott Tomsu — Founder, Advantage Design & Construction

## Identity
- **Name**: Scott Tomsu
- **Company**: Advantage Design & Construction (ADC)
- **Address**: 1201 SE Evangeline Thruway, Lafayette, Louisiana
- **Role**: Founder, principal operator, and technical lead across all ADC projects

## Professional Background

### ROV / Subsea Operations (25+ Years)
- Title: ROV Superintendent (most recent and primary career identity)
- Specialty: Deep-water remotely operated vehicle operations
- Geographic experience: West Africa, Gulf of America, worldwide deployments
- Scope: Full lifecycle ROV ops from mobilization through offshore deployment, maintenance, and recovery
- Certifications: IMCA ROV Supervisor (International Marine Contractors Association)
- Context: This background directly informs ADC's offshore deployment strategy and credibility with platform operators

### Commercial Diving
- School: Divers Academy (commercial diving program)
- Scope: Subsea commercial work, inspection, construction support
- Relevance: Deepwater environment expertise, understanding of offshore/marine infrastructure

### Aviation
- Certificate: FAA Private Pilot — fixed wing
- Instrument rating: Written exam passed at 90% — practical checkride pending
- Relevance: KLFT 1.1 (autonomous airspace hub, Lafayette Regional Airport) is informed by this background

### Motorsports
- Achievement: Multi-time drag racing champion
- Technical certification: FuelTech engine management certified
- Relevance: FuelTech is a Brazilian high-performance engine management system used in top-tier drag racing. Certification = advanced ECU programming, tuning, data acquisition. Reflects deep technical fluency with embedded systems and high-performance hardware.

### Electronics / Technical
- Title: Electronic Technician
- Scope: Electronics assembly, diagnostics, systems integration
- Context: Foundational technical skills that underpin all ADC hardware/systems work

## ADC — Company Background
- Founded: 2003 (Advantage Design & Construction, Inc.)
- ADC-3K division launched: 2025/2026 (use this framing for NVIDIA Inception application — NOT the 2003 founding date)
- Core business evolution: From construction/design to AI compute infrastructure
- The ADC 3K pod product line represents the convergence of Scott's construction, engineering, and compute expertise

## Why This Background Matters for ADC
- **Offshore AI**: 25 years on rigs and platforms = direct customer relationships and credibility for offshore pod deployment
- **Lights-out operations**: ROV ops are remote, unmanned, and mission-critical — same model as ADC 3K pod NOC
- **Rugged hardware mindset**: Designing for subsea and marine environments = immersion cooling, sealed enclosures, no-HVAC pods
- **Regulatory fluency**: Offshore safety (IMCA, ATEX) and FAA environments = ADC knows how to operate in regulated infrastructure spaces
- **FuelTech/ECU background**: Embedded systems + real-time data = GPU compute and engine management share the same technical DNA

## Contact / Identity Points
- Email domain: adhscott@yahoo.com (Vercel account)
- Cloudflare account: gofast@stfumotorcycles.com
- Location: Lafayette, Louisiana (Acadiana region — Cajun country, French cultural heritage)
