...
```
               ...
               10 GbE mgmt        CAN / GPIO heartbeat
                       │                │
     ┌─────────────────┴──────┐   ┌─────┴──────────────┐
     │  BMS network switch    │   │  Watchdog (HW)      │──→ safe-state relay
     │  (mgmt)                │   │                     │
     └──┬─────────────┬───────┘   └─────────────────────┘
        │             │
        ...
        └──→ I/O module chain (DIN rail in ECP panel)
              ├──→ AI module(s) — T/RH, pressure, CO₂, sump level analog
...
| Targets | 9 compute racks in the Cassette (R1–R9 per Cassette-ELEC-001 Rev 1.3) |
...
| TT-Z-DRIFT | WARN | Any TraceTek zone resistance down > 20% from 7-day rolling baseline | — | 600 s | Log + SCADA (pre-seepage indicator) |
...
2. **t ≤ 10 s:** Actuators reach closed limit; ZS-MIV-S-CLOSED and ZS-MIV-R-CLOSED assert (Belimo DN125 nominal close time ≤ 15 s worst case)
...
Factory acceptance test report (FAT) signed off by Scott Tomsu before ECP ships.
...
| S-15 | 7-day soak at representative IT load with no unexplained alarms | Soak log reviewed and signed |
