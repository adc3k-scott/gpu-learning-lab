...
```
     ...
     │   mgmt       │  │ -OT │ │ │  OT            │  │ Safety              │
     │              │  │     │ │ │                │  │                     │
     │ OPC-UA       │  │Modb │ │ │IPMI/Redfish    │  │ (PHYSICAL —         │
     │ server       │  │ CDU │ │ │NVIDIA BMCs     │  │  no network)        │
     │ HMI          │  │skid │ │ │(10×)           │  │ Watchdog,           │
     │ SNMP mgr     │  │ PLC │ │ │                │  │ MIV, fire,          │
     │              │  │     │ │ │                │  │ E-stop DI/DO        │
     └──────────────┘  └─────┘ │ └────────────────┘  └─────────────────────┘
                                │
...
| **BMC** | NVIDIA rack BMC IPMI / Redfish traffic | Jetson BMC NIC, 9 × rack BMC ports |
...
| ACL-04 | BMC | Jetson BMC IP + 9 rack BMC IPs + QM9700 mgmt IPs | Permit | IPMI/Redfish/SNMP conduit C3 |
...
| Session logging | `auditd` records all `sshd` auth events + per-command `sudo` invocations |
...
| CDU skid Modbus | — | Modbus has no auth; no credential to store |
...
| **Recover** | Options, from lightest to heaviest: (a) Restart `adc-bms` service; (b) Rollback to previous OCI release (§8.4); (c) Rotate affected credentials (device certs, SNMPv3 secret, BMC credentials); (d) Attended gold-image re-flash from a pre-staged spare eMMC; (e) Jetson module swap (spare kept on-site per platform spares policy) |
