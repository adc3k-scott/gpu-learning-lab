...
- All four gates satisfied (per M7) and `workload-enable-request` received — PRODUCTION
...
```
                                [cold boot]
                                     ...
                                     v
                              +-----------+
                              |   INIT    |--[any step fails 3x]--> INIT-FAULT (held)
                              +-----+-----+
                                    | [I-01..I-13 pass, <=120 s]
                                    v
 +---------------------------+  +---------------+
 | STANDBY-MAINT-GENERAL     |  |  STANDBY-IDLE |<---+
 | STANDBY-MAINT-COOLANT     |<>|               |    |
 +---------------------------+  +-------+-------+    |
   (enter/exit-maintenance)           |   ^           | non-fire
                                      |   |           | §6.4 reset
                              start-  |   | stop-cool |
                              cool    |   |           | +-----------------------+
                                      v   |           +-| STANDBY-FIRE-RECOVERY |
                              +----------+              | (§9.2, 5 gates)       |
                              |  READY   |              +-----------^-----------+
                              +----+-----+                fire-origin: gates 1,2
                                   |  ^
              wl-enable-request    |  | stop-workload
              + all 4 gates >=10s  |  |
                                   v  |
                             +------------+
                             | PRODUCTION |
                             +-----+------+
                                   | ALARM (past TTA+deadband)
                                   v
                             +-----------+
                             | DEGRADED  |
                             +-----+-----+
                                   | all ALARMs clear >=30 s
                                   v
                             [PRODUCTION]


  CRITICAL (from any non-E-STOP mode) / watchdog / maintenance-isolation:
                                   |
                                   v
                          +------------------+
                          |   SAFE-STATE     |
                          ...
                          +--------+---------+
                           fire /     \ non-fire
                          origin        \
                              |        §6.4 reset -> STANDBY-IDLE
                              |
                    STANDBY-FIRE-RECOVERY (§6)
                    -> STANDBY-IDLE when all 5 gates complete


  E-STOP: any mode + [ESTOP-DI | e-stop-request | POWER-OK loss >=3 s]
                                   |
                                   v
                          +------------+
                          |   E-STOP   |
                          +-----+------+
                                | 480V restored + POWER-OK
                                | + ESTOP-DI clear + NOC ack
                                v
                          STANDBY-IDLE
                     (if UPS depleted: cold boot -> INIT -> STANDBY)
