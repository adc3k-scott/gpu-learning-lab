# LinkedIn Outreach — CERAWeek 2026 + Supply Chain Moat
## Send from: Scott Tomsu (scott@adc3k.com)

---

## SECTION 1 — CERAWEEK CONTACTS

### 1. Marc Spieler — NVIDIA Sr Managing Director, Energy

**Connection request note (300 char max):**
Marc — Scott Tomsu, ADC founder in Lafayette LA. We're building 800V DC solar-direct AI factories to NVIDIA DSX spec. 3 Louisiana sites, behind-the-meter gas+solar. Jim Hennessy is getting us into NPN. Would value 10 minutes on the Louisiana opportunity.

**Follow-up after connect:**
Marc — appreciate the connection. ADC is building Louisiana's first American-owned neocloud — token factory on NVIDIA hardware, Henry Hub natural gas at $0.027/kWh, 800V DC from day one. Our entire supply chain is US-manufactured: First Solar panels from New Iberia (30 miles), Caterpillar gensets from Louisiana Cat dealers, Eaton Beam Rubin DSX, Fibrebond containers from Minden LA.

Three sites: Trappeys AI Factory (Lafayette, 2 MW solar, 112,500 sq ft), MARLIE I command center (owned, live), Willow Glen Terminal (St. Gabriel, 100+ MW former Entergy station).

CERAWeek confirmed what we already designed — 800V DC, behind-the-meter, tokens-per-watt. We'd welcome a 15-minute call on how ADC fits the Louisiana piece of NVIDIA's 50-state plan.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

### 2. Chad Zamarin — Williams Companies, President & CEO
*LinkedIn: linkedin.com/in/chad-zamarin-0933048/*

**Connection request note (300 char max):**
Chad — Scott Tomsu, ADC founder in Lafayette LA. Building AI factories on Henry Hub corridor — behind-the-meter gas backbone, $0.027/kWh. ATMOS confirmed on our sites. Williams' $7B Power Innovation push is exactly our Layer 2 at scale. Louisiana operator, American supply chain.

**Follow-up after connect:**
Chad — ADC is a Louisiana-based neocloud building AI factories powered by behind-the-meter natural gas. We're 40 miles from Henry Hub with ATMOS Energy confirmed on our Lafayette sites. Our power hierarchy: solar offset → gas backbone → diesel emergency → grid sell-back only. We never buy from the grid.

Williams' Power Innovation program is exactly what ADC needs at scale. We're at $5M seed now with MARLIE I already owned and Trappeys acquisition in progress. As we move to 30+ MW at Trappeys and 100+ MW at Willow Glen (former 2,200 MW Entergy station, St. Gabriel), behind-the-meter gas supply is the critical unlock.

Would welcome a conversation about Louisiana site opportunities and how Williams fits into ADC's Phase 2 power plan.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

### 3. Todd Farmer — Caterpillar VP Electric Power Division

**Connection request note (300 char max):**
Todd — Scott Tomsu, ADC founder. Building NVIDIA DSX AI factories in Louisiana with G3500 natural gas gensets as our Layer 2 backbone. American-made from the ground up — Louisiana Cat dealers, First Solar New Iberia, Fibrebond Minden. Nscale signed Cat for Monarch. We're the Louisiana version.

**Follow-up after connect:**
Todd — ADC specs Cat G3500 series as the backbone of our 4-layer power hierarchy at every site. Layer 2 carries the main AI compute load 24/7 — that's Cat. Layer 1 (solar) offsets it, Layer 3 (diesel) backs it up, Layer 4 (grid) is sell-back only.

Our supply chain is built entirely on American manufacturers. Cat is in every site plan: Trappeys (Lafayette), MARLIE I (Lafayette), Willow Glen (St. Gabriel). Louisiana Cat dealer network is already identified. This isn't a future consideration — Cat is spec'd in the engineering package.

We're at $5M seed stage. Would welcome a conversation about Louisiana deployment support and Cat's AI infrastructure program.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

### 4. Angie McMillin — Eaton, President of Energy Solutions & Services
*LinkedIn: linkedin.com/in/angie-mcmillin/ — announced Beam Rubin DSX at GTC 2026*

**Connection request note (300 char max):**
Angie — Scott Tomsu, ADC founder in Lafayette LA. We spec'd Beam Rubin DSX into all three of our Louisiana AI factory sites. 800V DC native, solar-direct, behind-the-meter gas. adc3k.com references Eaton by name throughout our architecture. Ready to talk deployment timeline.

**Follow-up after connect:**
JP — ADC has Eaton Beam Rubin DSX in the architecture on all three Louisiana sites: facility-level 800V DC switchgear, PDUs, busway, UPS, and microgrid controller. We build to NVIDIA DSX spec from day one — Eaton is NVIDIA's official power partner, so this was a straightforward call.

Dual power stack: Eaton handles facility power, Delta handles rack power. Both on every drawing.

Phase 1A is MARLIE I (Lafayette, owned) — Phase 1C is Trappeys (112,500 sq ft, 2 MW solar, water tower cooling). We're at $5M seed stage and moving toward hardware procurement. Would welcome a conversation about Eaton's Louisiana support and lead times on DSX components.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

### 5. EPRI — DCFlex / Flex MOSAIC
*Contact: DCFlex@epri.com — launched Flex MOSAIC at CERAWeek March 23, 2026*

**Connection request note (300 char max):**
[Name] — Scott Tomsu, ADC in Lafayette LA. Building behind-the-meter AI factories with DSX Flex integration. Your micro AI factory work with NVIDIA + Prologis maps directly to our ADC 3K Pod. Interested in Flex MOSAIC certification for our Louisiana sites.

**Follow-up after connect:**
[Name] — ADC's ADC 3K Pod is a DSX-compliant facility module designed around the same micro AI factory concept EPRI is developing with NVIDIA and Prologis. Containerized, deployable, behind-the-meter gas or flare gas powered.

Louisiana flares 1.8 BCF/year. We deploy ADC 3K pods at well sites — waste gas in, GPU-hours out, Flex MOSAIC certified. The regulatory tailwind is there: January 2024 Louisiana rule requires evaluation of alternative beneficial uses before flaring permits are issued. We're the beneficial use.

Would welcome a conversation about the Flex MOSAIC pathway for Louisiana deployments.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

### 6. Varun Sivaram — Emerald AI CEO

**Connection request note (300 char max):**
Varun — Scott Tomsu, ADC founder in Louisiana. 3 behind-the-meter AI factories, 800V DC solar-direct, gas backbone. DSX Flex already in our architecture. Your Conductor platform is the grid-flex orchestration layer we need for Louisiana incentive compliance. Let's connect.

**Follow-up after connect:**
Varun — ADC is building Louisiana's first American-owned neocloud. Our power stack is purpose-built for behind-the-meter: solar + gas + diesel, 800V DC, grid sell-back only. Conductor fits as the orchestration layer for MISO grid-flex ancillary revenue (~$4M/yr at scale).

The Louisiana opportunity: $50B flowing in, zero local operators. We own one site, acquiring a second, pipeline to 100+ MW at Willow Glen Terminal (former 2,200 MW Entergy station). ADC is the Louisiana operator Emerald needs for a Gulf South reference site.

Would value 20 minutes on how Conductor integrates with DSX Flex in a Louisiana behind-the-meter deployment.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

### 7. Susan Bourgeois — Louisiana Secretary of Economic Development (LED)

**Connection request note (300 char max):**
Secretary Bourgeois — Scott Tomsu, ADC founder in Lafayette. Louisiana-owned AI factories — $50B flowing into LA, zero locally owned. 3 sites, American supply chain throughout. 800V DC, First Solar New Iberia, Cat gensets, Fibrebond Minden. Ready to brief your team on the ITEP and Act 730 pathway.

**Follow-up after connect:**
Secretary Bourgeois — ADC is the Louisiana answer to $50B in foreign and out-of-state AI infrastructure investment. Hut 8 (Canada), Nscale (UK), Amazon, Meta — every dollar of profit leaves the state. ADC's profit stays in Louisiana.

Three sites: Trappeys AI Factory (Lafayette), MARLIE I command center (Lafayette, owned), Willow Glen Terminal (St. Gabriel, 100+ MW former Entergy station). American supply chain: First Solar panels from New Iberia (30 miles), Caterpillar from Louisiana Cat dealers, Fibrebond containers from Minden, Cajun Industries from New Iberia.

We're pursuing ITEP (emerging industry classification — Kristin Johnson contact), Act 730, and PILOT. We'd welcome a briefing with your team and an introduction to the UL Lafayette partnership through LEDA.

Scott Tomsu | Founder, ADC | scott@adc3k.com | adc3k.com

---

## SECTION 2 — SUPPLY CHAIN MOAT OUTREACH

*ADC's entire vendor stack is US-manufactured. These are direct outreach to those partners.*

---

### 8. First Solar — Business Development (New Iberia, LA)
*Search LinkedIn for First Solar business development — New Iberia plant managers are the right contacts*

**Connection request note (300 char max):**
[Name] — Scott Tomsu, ADC founder in Lafayette. We spec'd First Solar Series 7 TR1 panels into our Trappeys AI Factory — 3,731 panels, 2.05 MW, 800V DC-direct to rack. Your New Iberia plant is 30 miles from our site. American-made solar powering American-built AI infrastructure.

**Follow-up after connect:**
[Name] — ADC's Trappeys AI Factory in Lafayette uses 3,731 First Solar Series 7 TR1 panels for 2.05 MW of rooftop solar. We wire DC-direct to an 800V bus — no inverter, no AC conversion, 97% panel-to-rack efficiency. First Solar is spec'd as our primary solar partner across all three Louisiana sites.

Your New Iberia factory is 30 miles from Trappeys. That's not a supply chain — that's a neighbor. American panels, American factory, American AI infrastructure. We think that story is worth telling together.

Would welcome a conversation about preferred customer pricing, installation support, and how First Solar wants to be positioned in our public materials.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

### 9. Graham Andrew Walker — Fibrebond President, Minden LA
*Phone: (318) 377-1030 | sales@fibrebond.com | 1300 Davenport Dr, Minden LA 71055*

**Connection request note (300 char max):**
[Name] — Scott Tomsu, ADC founder in Lafayette. We spec Fibrebond as our container partner for ADC 3K pods — NVIDIA DSX-compliant facility modules. Minden is 250 miles away. Louisiana-made containers for Louisiana AI infrastructure. American supply chain top to bottom. Let's connect.

**Follow-up after connect:**
[Name] — ADC's ADC 3K pod is a DSX-compliant facility module that receives NVIDIA standard liquid-cooled racks. We spec Fibrebond as the container manufacturer — US-made, Louisiana-built, military-grade construction. Minden to Lafayette is a one-day haul.

Our deployment model: pods ship from Fibrebond, get staged at our New Iberia facility, deploy to flare sites, edge locations, and campus AI factories across the Gulf South. First Solar panels on the roof, Cat gensets underneath, Eaton 800V DC distribution inside.

This is a Louisiana supply chain that doesn't exist yet. ADC is building it. Would love to discuss how Fibrebond fits into our Phase 1 procurement and beyond.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

### 10. Cajun Industries — New Iberia Assembly Yard (Port of Iberia)
*LinkedIn: linkedin.com/company/cajunindustries | HQ: 15635 Airline Hwy, Baton Rouge | (225) 754-0275*
*Search LinkedIn for Cajun Industries business development or project development contacts*

**Connection request note (300 char max):**
[Name] — Scott Tomsu, ADC in Lafayette. We're building AI factory infrastructure in Louisiana — Trappeys Cannery (112,500 sq ft), MARLIE I command center, Willow Glen Terminal (100+ MW). Cajun Industries is in our vendor list for site construction and steel fabrication. Louisiana built, American made.

**Follow-up after connect:**
[Name] — ADC is building three AI factory sites in Louisiana. Trappeys is the core project — 112,500 sq ft historic cannery, 2 MW solar, water tower cooling, infrastructure yard. MARLIE I is our owned command center in Lafayette. Willow Glen Terminal in St. Gabriel is a former 2,200 MW Entergy station targeting 100+ MW.

We want Louisiana contractors building Louisiana AI infrastructure. Cajun Industries is in our vendor matrix for site construction, structural steel, and fabrication work. Our timeline targets Trappeys acquisition this year with Phase 1 buildout following seed close.

Would welcome a conversation about capabilities and project fit for Phase 1.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

### 11. Metal Shark — Business Development (Jeanerette, LA)

**Connection request note (300 char max):**
[Name] — Scott Tomsu, ADC in Lafayette. We're developing offshore and wetland AI compute deployments in the Gulf South — Louisiana bayous and offshore platforms. Metal Shark's aluminum construction and Marine + Defense work aligns with our barge and platform deployment strategy. Louisiana to Louisiana.

**Follow-up after connect:**
[Name] — ADC deploys containerized AI compute (ADC 3K pods) at remote locations: wetland pilings in the Atchafalaya, offshore rigs in the Gulf of America, and barge-mounted platforms for deepwater operations. After 25 years as an ROV Superintendent in deepwater oil & gas, this is familiar territory.

Metal Shark's aluminum construction and military/government pedigree is exactly the build quality we need for marine and offshore deployments. Pods need to survive Gulf weather, salt air, and remote operations without on-site babysitting. That's the Jeanerette skill set.

Would love to explore what a barge-based or offshore deployment platform looks like from a Marine Shark perspective.

Scott Tomsu | scott@adc3k.com | adc3k.com

---

## SECTION 3 — SUPPLY CHAIN MOAT LINKEDIN POST

*Standalone post to publish — positions the supply chain as the competitive moat*

---

Everyone talks about AI infrastructure. Nobody talks about WHERE it comes from.

Hut 8 ($12.5B, Louisiana): Canadian company. Foreign ownership.
Nscale ($14.6B): London, UK headquarters.
Amazon, Meta: Seattle and Menlo Park.

Every dollar of profit leaves Louisiana.

ADC is different. We're Louisiana-owned. And our entire supply chain is American-made.

Here's what goes into an ADC AI factory:

**Solar panels** — First Solar, New Iberia, LA. 30 miles from our Trappeys site. Their plant makes Series 7 TR1 panels. We're wiring 3,731 of them DC-direct to an 800V bus. No inverter. 97% efficiency.

**Containers and modules** — Fibrebond, Minden, LA. 250 miles away. US-made, military-grade construction. Our ADC 3K pods ship from Minden, stage in New Iberia, deploy anywhere in the Gulf South.

**Natural gas gensets** — Caterpillar, Louisiana Cat dealer network. G3500 series. Layer 2 power backbone at every site. 24/7, Henry Hub pricing, $0.027/kWh.

**Power distribution** — Eaton (Cleveland, OH). Beam Rubin DSX — NVIDIA's official power partner. 800V DC switchgear, PDUs, UPS, microgrid controller. On every drawing.

**Site construction** — Cajun Industries, New Iberia, LA. Louisiana contractors building Louisiana AI infrastructure.

**Marine/offshore platforms** — Metal Shark, Jeanerette, LA. For wetland and Gulf deployments.

**The NVIDIA stack** — NVL72 racks, Dynamo inference OS, Run:AI scheduling. Everything to spec.

This supply chain doesn't exist yet as an integrated system. ADC is building it. Louisiana to Louisiana, American to American.

When a foreign company builds AI infrastructure here, they bring their own supply chain. When ADC builds, the money stays in Louisiana — from the solar panels to the containers to the gas gensets to the construction crews.

That's not just a business model. That's the moat.

$5M seed round. 35% Louisiana Angel Tax Credit on investment.
adc3k.com | scott@adc3k.com | (337) 780-1535

#Louisiana #AmericanMade #AIFactory #SupplyChain #FirstSolar #Caterpillar #Eaton #NVIDIA #Neocloud #Lafayette

---

## FOLLOW-UP MESSAGE (send after connection accepted):

Thank you for connecting. I founded ADC — Advantage Design & Construction — in Lafayette, Louisiana. We're building NVIDIA DSX-compliant AI factories with a 4-layer behind-the-meter power hierarchy: solar primary, natural gas backbone, diesel emergency, grid sell-back only.

Three sites in development:
- Trappeys AI Factory (Lafayette) — 112,500 sq ft, 2 MW solar, proof of concept
- Tiger Compute Campus at Willow Glen (St. Gabriel) — 100+ MW, Mississippi River cooling
- MARLIE I Command Center (Lafayette) — 576 GPUs, backup NOC

Everything validated at CERAWeek 2026 — 800V DC, behind-the-meter consensus, tokens-per-watt as the metric. We're American-owned with American-manufactured equipment throughout.

Full site plans and engineering packages live at adc3k.com.

I'd welcome 15 minutes to discuss how [COMPANY] and ADC can work together in Louisiana.

Scott Tomsu
Founder, ADC
scott@adc3k.com | adc3k.com

---

## INVESTOR CONNECTION TEMPLATES

### For Angel Investors / HNW Individuals

**Connection request (300 char max):**
[Name] — Scott Tomsu, founder of ADC in Lafayette LA. We're building Louisiana's first American-owned AI factory. $5M seed round, 35% Louisiana Angel Tax Credit. Self-hosted AI agent live at ai-advantage.info. Would love to share the deck.

**Follow-up message:**
Thanks for connecting. ADC manufactures inference tokens on NVIDIA hardware, powered by Henry Hub natural gas (40 miles, $0.027/kWh). We own one site, acquiring a second. ROXY — our 27-tool AI agent — is running on our own GPU right now. Call her: (337) 486-3149.

Full pitch deck: adc3k.com/investor-pitch
Operations center: adc3k.com/operations-center

Louisiana Angel Tax Credit gives you 35% back on day one. $1M invested costs you $650K.

Happy to walk through the deck or schedule a site visit to Lafayette. scott@adc3k.com / (337) 780-1535

---

### For Ornn Investor Network (Crucible Capital, Vine Ventures, Link Ventures, Box Group)

**Connection request:**
[Name] — Scott Tomsu, ADC founder. We're building token factories in Louisiana on Henry Hub gas at $0.027/kWh. You invested in the financial layer for compute (Ornn). We're the physical layer. Self-hosted AI agent live today. Would love to connect.

**Follow-up:**
ADC manufactures inference tokens on NVIDIA hardware in Louisiana. We see Ornn's OCPI as the benchmark we'll price against, and RVS as the tool that makes our hardware bankable for lenders. We're raising $5M seed — 35% Louisiana Angel Tax Credit.

Our AI agent ROXY has 27 tools and runs on our own GPU. Call her: (337) 486-3149. Pitch deck: adc3k.com/investor-pitch

---

### For Oil & Gas Investors / Operators (Flare Gas Angle)

**Connection request:**
[Name] — Scott Tomsu, ADC in Lafayette LA. 25 years offshore ROV operations. Now building AI compute that runs on stranded gas at well sites. Crusoe proved the model at $10B. They left Louisiana wide open. Would love to discuss.

**Follow-up:**
ADC deploys containerized AI compute at flare sites — converting waste gas into GPU-hours at $0 fuel cost. Louisiana flares 1.8 BCF/year. We've identified 5 operators within 30 miles of our Lafayette facility with 593,000 MCF/month of gas production.

The pitch to operators: your waste gas becomes AI compute revenue. You stop flaring, we run GPUs. Zero cost to you.

Deck: adc3k.com/investor-pitch | scott@adc3k.com | (337) 780-1535

---

### For VCs / Infrastructure Funds

**Connection request:**
[Name] — Scott Tomsu, founder ADC. Building neoclouds in Louisiana — token factories on Henry Hub gas at $0.027/kWh. Crusoe model + local operator advantage. $5M seed, 35% state tax credit. 27-tool AI agent live on self-hosted GPU today.

**Follow-up:**
ADC is the only American-owned, Louisiana-based AI infrastructure operator. $50B is flowing into Louisiana AI — zero is locally owned. We own one site, acquiring a second. Full NVIDIA DSX spec, 800V DC architecture, 22 US vendors.

Neocloud comparables: CoreWeave $71B, Nebius $24B, Crusoe $10B+. We're pre-revenue at $5M seed with the cheapest power in America and a working AI agent proving the software layer.

Pitch deck: adc3k.com/investor-pitch
Call ROXY: (337) 486-3149
scott@adc3k.com | (337) 780-1535

---

### For Hospital / Healthcare Contacts

**Connection request:**
[Name] — Scott Tomsu, ADC in Lafayette. We're deploying NVIDIA GPU compute infrastructure for medical AI — HIPAA-compliant, on-premise, local. Hospitals need local compute for radiology AI, clinical notes, insurance verification. Would love to discuss.

---

### For University / Education Contacts

**Connection request:**
[Name] — Scott Tomsu, ADC in Lafayette. We're building AI compute infrastructure for Louisiana universities — CUDA platform access, research computing, workforce development. 46 universities already registered at louisianaai.net. March 31 NVIDIA Academic Grant deadline approaching.

---

### For Governor's Office / State Government

**Connection request:**
[Name] — Scott Tomsu, ADC in Lafayette. Louisiana-owned AI infrastructure company. $50B flowing into LA for AI — zero locally owned. We're building the Louisiana version of what UF did with HiPerGator. 70 school districts + 46 universities registered at louisianaai.net. Would value a conversation.
